...
LEADER_DATA = {
    ...
    "EN": {
        "Leadership": {
            "Core Competency": {
                "Customer Focus": "Actively identify and timely respond to the needs of customers.",
                "Responsibility": "Act systematically to achieve goals even without instructions.",
            ...
            "Execution Competency": {
                ...
                "Problem Solving": "Identify root causes by analyzing information and data.",
                "Org Insight": "Identify the organization's strategy, operating methods, and history.",
                "Project Mgmt": "Establish plans by collecting and analyzing information."
            },
            "Professional Competency": {
                "Analytical Thinking": "Identify what information or data is needed to solve a problem.",
                "Detailed Handling": "Investigate regulations or past practices to minimize problems."
            }
        }
    }
}
...
if user_db is not None:
    if not st.session_state.auth:
                ...
                with st.expander(f"📂 {major}", expanded=True):
                    for sub, items in sub_items.items():
                        ...
                        for it, crit in items.items():
                            ...
                            label_html = f"**{it}**"
                            # 본인 입력 정보 노출 (2차/3차 모두 노출로 변경)
                            if self_data_dict and it in self_data_dict:
                                s_score = self_data_dict[it]['score']
                                s_basis = self_data_dict[it]['basis']
                                label_html += f"<br><span style='color:blue; font-size: 0.85em;'>[{L['self_info']}]</span><br><span style='color:blue; font-size: 0.85em;'>점수: {s_score}</span>"
                                if s_basis: label_html += f"<br><span style='color:gray; font-size: 0.75em;'>ㄴ근거: {s_basis}</span>"
        ...
        # 2차 & 3차 팀원평가
            ...
            if target_self_rows.empty:
                st.warning("Target employee has not submitted yet.")
            else:
                # 2차와 3차 모두 본인 점수/근거 노출
        ...
        # 대시보드
            ...
            target_user = st.selectbox("Select Employee", user_db['성명'].tolist())

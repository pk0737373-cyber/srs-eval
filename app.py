...
from streamlit_gsheets import GSheetsConnection # [수정] 올바른 임포트 경로
import pandas as pd
import datetime
...
EVAL_DATA = {
    "KO": {
        ...
        "3. 직무능력": {
            ...
            "이해판단력": {"신속성": "이해 속도?", "타당성": "결론의 타당성?", "문제해결": "해결 주도?", "통찰력": "자주적 결론?"},
            "창의연구력": {"연구개선": "창의적 개선 도모?"},
...
LEADER_DATA = {
    "KO": {
        "1. 리더십": {
            ...
            "실행역량": {"의사소통": "친밀감 형성", "문제해결": "원인 규명", "조직이해": "전략 파악", "프로젝트관리": "체계적 수립"}
        }
    }
}
...
UI = {
    "KO": {
        ...
        "err_null": "⚠️ [제출 불가] 점수 선택 및 근거(5자 이상)를 작성하세요.",
        "already": "✅ 최종 제출 완료 (조회 모드)", "logout": "🔓 로그아웃", "score": "점수", "basis": "근거"
    }
}
...
def render_engine(data_dict, pre, eval_type="자기", ws_name="Results", target_name=None):
    ...
    if is_final_done:
        ...
        with st.form(key=f"f_main_{pre}_{target_name}"):
            res_dict = {}
            ...
            if col1.form_submit_button(L["save"]) or col2.form_submit_button(L["sub"]):
                ...
                if is_f and has_null: st.error(L["err_null"])
                else:

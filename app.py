...
LEADER_DATA = {
    "KO": {
        "Leadership_리더십": {
            ...
            "LD_책임감": "특별한 지시를 하지 않더라도 업무목표를 달성하기 위해 계획적으로 행동한다",
...
UI = {
    "KO": {"m1": "📝 자기고과 작성", "m2": "👥 2차 팀원평가", "m3": "⚖️ 3차 최종평가", "m4": "📊 관리자 대시보드", "m5": "🚀 리더십 자기평가", "m6": "🎖️ 2차 리더십평가", "sub": "✅ 최종 제출", "already": "✅ 제출 완료된 항목입니다.", "err": "⚠️ 모든 항목의 점수와 근거를 작성해야 제출할 수 있습니다.", "score": "점수", "basis": "근거", "target": "평가 대상 선택", "pw_reset": "🔑 비밀번호 재설정"}
}
    ...
    if not st.session_state.auth:
        ...
        if st.button("Login"):
            ...
            if not u.empty and str(pw) == str(u.iloc[0]['비밀번호']):
                ...
                st.session_state.update({'auth':True,'user':name,'is_leader':is_ldr})
                st.rerun()
                    ...
                    for sub, items in sub_items.items():
                        ...
                        for it, crit in items.items():
                            ...
                            if self_data and it in self_data:
                                label += f"<br><span style='color:blue; font-size: 0.8em;'>[본인점수: {self_data[it]}]</span>"
                            c1.markdown(label, unsafe_allow_html=True)
                            c2.caption(crit)
                            score = c3.selectbox(L["score"], [1,2,3,4,5], key=f"{pre}_{it}_s")
                            basis = c4.text_input(L["basis"], key=f"{pre}_{it}_r", placeholder="근거 필수 입력")
        ...
        if menu == L["m1"]:
            ...
            if res_df is not None and not res_df[(res_df['피평가자'] == user_name) & (res_df['구분'] == "자기")].empty:
                ...
                r1 = st.text_area("1. 금기 성과 요약")
                if st.button(L["sub"]):
                    if any(not v["basis"].strip() for v in eval_res.values()) or not r1.strip():
                        st.error(L["err"])
                    else:
        ...
        # --- 5. 리더십 자기평가 ---
            ...
            if ld_df is not None and not ld_df[(ld_df['피평가자'] == user_name) & (ld_df['구분'] == "자기")].empty:
                ...
                eval_res = render_form(LEADER_DATA["KO"], "ldr_self")
                if st.button(L["sub"]):
                    if any(not v["basis"].strip() for v in eval_res.values()):
                        st.error(L["err"])
                    else:
                        now = (datetime.datetime.now() + timedelta(hours=9)).strftime("%Y-%m-%d %H:%M")
                        recs = [{"시간": now, "평가자": user_name, "피평가자": user_name, "구분": "자기", "항목": k, "점수": v["score"], "근거": v["basis"]} for k,v in eval_res.items()]
        ...
        # --- 6. 2차 리더십 평가 (이사님용) ---
            ...
            if not ldr_targets: st.warning("평가할 리더 대상이 없습니다.")
                ...
                eval_res = render_form(LEADER_DATA["KO"], f"eval_ld2_{target}", self_scores)
                if st.button(f"{target}님 리더십 평가 제출"):
                    if any(not v["basis"].strip() for v in eval_res.values()): st.error(L["err"])
                        ...
                        if save_data(recs, "Leadership_Results"): st.success("제출 완료!"); st.balloons()
        ...
        # --- 2차/3차 팀원평가 ---
            ...
            if target_self.empty: st.warning("대상이 아직 자기고과를 제출하지 않았습니다.")
            else:
                self_scores = dict(zip(target_self['항목'], target_self['점수']))
                eval_res = render_form(EVAL_DATA["KO"], f"eval_{mode}_{selected}", self_scores)
                if st.button(f"{mode} 평가 제출"):
                    if any(not v["basis"].strip() for v in eval_res.values()): st.error(L["err"])
                        ...
                        if save_data(recs): st.success("제출 완료!"); st.balloons()
        ...
        # --- 대시보드 및 비번변경 ---
            ...
            st.divider()
            st.subheader(L["pw_reset"])
            target_user = st.selectbox("직원 선택", user_db['성명'].tolist())
            ...
            if st.button("비밀번호 즉시 변경"):
                ...
                if conn.update(worksheet="Users", data=user_db):
                    st.success(f"{target_user}님 비밀번호 변경 완료!"); st.rerun()
